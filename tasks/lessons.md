...
Batch API calls to avoid rate limiting — Making 7-10 parallel calls to external APIs (like Odds API) triggers rate limits (429), causing subsequent requests to fail and return empty data. Batch calls (max 3 concurrent) with small delays (100ms) between batches. Also align cache TTLs across routes (30 min for all) so they don't expire at staggered times causing burst refreshes.
